...
export default function useActiveAccount() {
    ...
    const validateAndActivate = async (): Promise<void> => {
        ...
        // [TESTE]  Token de ativação expirado
        /*
        ...
        */
        ...
        try {
            // 1. Valida o token
            const validateResponse = await fetch(`${apiUrl}/auth/activate/validate?token=${encodeURIComponent(token)}`, {
                method: 'GET',
                cache: 'no-store',
            });
            ...
            const activateResponse = await fetch(`${apiUrl}/auth/activate?token=${encodeURIComponent(token)}`, {
                method: 'POST',
                cache: 'no-store',
            });
