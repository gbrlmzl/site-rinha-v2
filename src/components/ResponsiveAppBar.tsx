            ...
            return (
                <Tooltip title="Abrir menu do usuário">
                    <IconButton onClick={handleOpenUserMenu} sx={{ p: 0, display: { xs: 'none', md: 'flex' } }}>
                        <Avatar alt={user.nickname} src={user.profilePic || undefined}
                            sx={{ width: 38, height: 38, bgcolor: '#11B5E4', border: '2px solid #11B5E4', fontSize: '1rem', fontWeight: 'bold' }}>
            ...
            return (
                <Box sx={{ pt: 3, width: '100%', display: 'flex', justifyContent: 'center' }}>
                    <Link href="/login" style={{ textDecoration: 'none', color: 'inherit' }}>
                        <Button variant="contained" sx={{ width: '100%', borderRadius: 8, paddingInline: 6 }}
        ...
        return (
            <Link href="/login" style={{ textDecoration: 'none', color: 'inherit' }}>
                <Button variant="contained" sx={{
                    display: { xs: 'none', md: 'flex' }, width: 150, borderRadius: 4,
                    backgroundColor: '#11B5E4', '&:hover': { backgroundColor: '#0b80a0' },
                }}>
                    Fazer Login
                </Button>
            </Link>
        ...
        >
            <Box sx={{ width: '100%', paddingInline: { xs: 1, md: 3 } }}>
                <Toolbar disableGutters sx={{ justifyContent: 'space-between' }}>
                    ...
                    {/* === MOBILE: logo === */}
                    <IconButton size="large" edge="start" color="inherit" aria-label="abrir menu"
                        onClick={handleOpenNavMenu} sx={{ display: { xs: 'flex', md: 'none' }, mr: 2 }}>
                    ...
                    <Box sx={{ display: { xs: 'flex', md: 'none' }, alignItems: 'center', gap: 1, mr: 1 }}>
                        ...
                        <IconButton size="large" edge="end" color="inherit" aria-label="abrir menu de opções"
                    ...
                    <Drawer anchor="left" open={isNavDrawerOpen} onClose={handleCloseNavMenu}
                        ModalProps={{ keepMounted: true }}
                        sx={{
                            display: { xs: 'block', md: 'none' },
                            zIndex: DRAWER_Z_INDEX,
                            '& .MuiBackdrop-root': { zIndex: DRAWER_Z_INDEX },
                            '& .MuiDrawer-paper': { zIndex: DRAWER_Z_INDEX + 1, width: 280, backgroundColor: '#0E1241', color: 'white' },
                        }}>
                        <Box><IconButton onClick={handleCloseNavMenu}><img src="/logoMenuUp.svg" alt="" width={100} /></IconButton></Box>
                        ...
                        <Box role="presentation" sx={{ pt: 2 }}>
                            {games.map((game) => (
                                <MenuItem key={game.title} onClick={handleCloseNavMenu}>
                                    <Link href={game.url} style={{ textDecoration: 'none', color: 'inherit', width: '100%' }}>{game.title}</Link>
                                </MenuItem>
                            ))}
                        </Box>
                    </Drawer>
                    ...
                    <Drawer anchor="right" open={isNavDrawerOptionsOpen} onClose={handleCloseNavOptions}
                        ModalProps={{ keepMounted: true }}
                        sx={{
                            display: { xs: 'block', md: 'none' },
                            zIndex: DRAWER_Z_INDEX,
                            '& .MuiBackdrop-root': { zIndex: DRAWER_Z_INDEX },
                            '& .MuiDrawer-paper': { zIndex: DRAWER_Z_INDEX + 1, width: '100%', backgroundColor: '#0E1241', color: 'white' },
                        }}>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingInline: 3 }}>
                            <IconButton sx={{ mt: 1 }}><img src="/logo.svg" alt="" width={60} /></IconButton>
                            <CancelRoundedIcon onClick={handleCloseNavOptions} fontSize="large" sx={{ cursor: 'pointer' }} />
                        </Box>
                        {!isLoading && isAuthenticated && user && (
                            <Box sx={{
                                display: 'flex', alignItems: 'center', gap: 2, px: 3, py: 2, mt: 1,
                                ...
                                <Box>
                                    ...
                                    <Typography sx={{ fontSize: '0.75rem', color: 'rgba(255,255,255,0.6)' }}>{user.email}</Typography>
                                </Box>
                            </Box>
                        )}
                        ...
                        {!isLoading && isAuthenticated && (
                            <Box sx={{ pt: 2, pl: 2 }}>
                                ...
                                <MenuItem onClick={() => { handleCloseNavOptions(); router.push('/perfil'); }}>
                                    <ListItemIcon sx={{ color: 'white', minWidth: 40 }}><AccountCircleIcon sx={{ fontSize: '1.3rem' }} /></ListItemIcon>
                                    Minha conta
                                </MenuItem>
                                <MenuItem onClick={() => { handleCloseNavOptions(); router.push('/configuracoes'); }}>
                                    <ListItemIcon sx={{ color: 'white', minWidth: 40 }}><SettingsIcon sx={{ fontSize: '1.2rem' }} /></ListItemIcon>
                                    Configurações
                                </MenuItem>
                                <MenuItem onClick={handleLogout} sx={{ color: '#ff6b6b' }}>
                                    <ListItemIcon sx={{ color: '#ff6b6b', minWidth: 40 }}><LogoutIcon sx={{ fontSize: '1.2rem' }} /></ListItemIcon>
                                    Encerrar sessão
                                </MenuItem>
                            </Box>
                        )}
                    ...
                    <Drawer anchor="top" open={isNavDrawerOpen} onClose={handleCloseNavMenu}
                        ModalProps={{ keepMounted: true }}
                        sx={{
                            display: { xs: 'none', md: 'block' },
                            zIndex: DRAWER_Z_INDEX,
                            '& .MuiBackdrop-root': { zIndex: DRAWER_Z_INDEX },
                            '& .MuiDrawer-paper': { zIndex: DRAWER_Z_INDEX + 1, height: 300, backgroundColor: '#0E1241', color: 'white' },
                        }}>
                        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingInline: 2.5, pt: 1 }}>
                            <IconButton onClick={handleCloseNavMenu}><img src="/logoMenuUp.svg" alt="" width={100} /></IconButton>
                            <Button onClick={handleCloseNavMenu} sx={{ color: 'white' }}><CancelRoundedIcon fontSize="large" /></Button>
                        </Box>
                        <Box sx={{ background: '#0a247c', p: 1 }}>
                            <Typography sx={{ fontWeight: 'bold', fontSize: '1.25rem', pl: 2 }}>JOGOS</Typography>
                        </Box>
                    ...
                    <Box sx={{ display: { xs: 'none', md: 'flex' }, alignItems: 'center', pr: 2 }}>
                        {isLoading && <Skeleton variant="circular" width={38} height={38} sx={{ bgcolor: 'rgba(255,255,255,0.15)' }} />}
                        {!isLoading && isAuthenticated && user && (
                            <Tooltip title="Abrir menu do usuário">
                    ...
                    <Menu sx={{ mt: 1 }} id="menu-appbar" anchorEl={anchorElUser}
                        anchorOrigin={{ vertical: 'bottom', horizontal: 'right' }}
                        keepMounted transformOrigin={{ vertical: 'top', horizontal: 'right' }}
                        open={Boolean(anchorElUser)} onClose={handleCloseUserMenu}
                        slotProps={{ paper: { sx: { backgroundColor: '#0E1241', color: 'white', border: '1px solid rgba(255,255,255,0.1)', minWidth: 200 } } }}>
                        {user && (
